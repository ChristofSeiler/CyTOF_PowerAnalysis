...
pkgs_needed = c("ggplot2","reshape2","tibble","dplyr","magrittr","stringr","devtools","readr","assertthat",
                "parallel","flowCore","MASS","cowplot","truncnorm","truncdist","ChristofSeiler/BatchJobs")
...
exprs_groups = simulate_two_groups(n_donors = 25,shifted_markers = shifted_markers)
...
```{r simulation_study}
simulation_study = function(n_donors) {
  cat(n_donors)
  target_FDR = 0.01
  shifted_markers = c("CD38","CD2","NKG2D","TIGIT","X_2B4")
...
set.seed(0xdada)
n_donors_seq = c(rep(5,10),rep(10,10),rep(15,10),rep(20,10),rep(25,10))
if(file.exists("simulation_study_log")) unlink("simulation_study_log", recursive=TRUE)
reg = makeRegistry(id = "simulation_study_log", file.dir = "simulation_study_log")
batchMap(reg, simulation_study, n_donors_seq)
submitJobs(reg)
waitForJobs(reg,sleep = 360)
res_list = reduceResultsList(reg)
experiments = res_list %>% bind_rows
# or use package parallel
#experiments = mclapply(c(rep(25,10),rep(5,10),rep(15,10),rep(20,10),rep(25,10)),
#                       simulation_study,
#                       mc.cores = 8) %>% bind_rows
write_csv(experiments,path = "experiments.csv")
experiments %>% 
  dplyr::group_by(n_donors,target_FDR) %>% 
  dplyr::summarize(TPR = mean(TPR),
                   FDR = mean(FDR))
```
